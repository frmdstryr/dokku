...
func TestPropertyGetAppJson(t *testing.T) {
	RegisterTestingT(t)
	Expect(setupTests()).To(Succeed())
	Expect(setupTestApp()).To(Succeed())
	textBefore := PropertyGet("nginx", testAppName, "client-max-body-size")
	Expect(textBefore).To(Equal(""))
	Expect(setupAppJson()).To(Succeed())
	textAfter := PropertyGet("nginx", testAppName, "client-max-body-size")
